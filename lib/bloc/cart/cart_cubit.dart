...
class CartCubit extends Cubit<CartState> {
  ...
  void artir({required int id}) {
    ...
    final yeniDurum = CartState(sepet: suankiSepet);
    emit(yeniDurum);
  }

  void azalt({required int id}) {
    var suankiSepet = state.sepet;

    if (suankiSepet.any((element) => element["id"] == id)) {
      if (suankiSepet.firstWhere((element) => element["id"] == id)["count"] > 1) {
        suankiSepet.firstWhere((element) => element["id"] == id)["count"]--;
      } else {
        suankiSepet.removeWhere((element) => element["id"] == id);
      }
